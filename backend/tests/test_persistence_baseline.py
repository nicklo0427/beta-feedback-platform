...
def persistent_database(tmp_path, monkeypatch) -> Generator[str, None, None]:
    ...
    monkeypatch.setenv("BFP_DATABASE_URL", f"sqlite+pysqlite:///{database_path}")
...
def test_persistence_schema_bootstrap_creates_core_tables(
...
def test_persistence_mode_keeps_core_flow_data_after_runtime_reset(
    ...
    with TestClient(app) as restarted_client:
        ...
        assert (
            restarted_client.get(f"/api/v1/campaigns/{campaign_id}/safety").json()["id"]
            == safety_id
        )
        assert (
            restarted_client.get(f"/api/v1/eligibility-rules/{eligibility_rule_id}").status_code
            == 200
        )
        assert (
